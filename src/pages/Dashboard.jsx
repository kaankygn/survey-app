    ...
    useEffect(() => {
        async function getir() {
                ...
                const liste = surveySnap.docs.map((doc) =>{
                    const count = responses.filter((r) => r.surveyId === doc.id).length
                    return { id: doc.id, ...doc.data(), responseCount: count }
                })
                setSurveys(liste)
        }
        getir()
        ...
        <div className="p-8">
            ...
            <div key={survey.id} className="border border-slate-200 rounded p-4 mt-4">
                ...
                <p className="text-sm text-slate-500 mt-1">
                    {survey.questions.length} soru · {survey.responseCount} yanıt
                </p>
